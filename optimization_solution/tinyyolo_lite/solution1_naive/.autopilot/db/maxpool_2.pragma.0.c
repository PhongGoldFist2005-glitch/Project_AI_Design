...
void maxpool_layer2(
    data_t input[29][29][16],
    data_t output[14][14][16]
){_ssdm_SpecArrayDimSize(input, 29);_ssdm_SpecArrayDimSize(output, 14);
    Channel: for(int ch = 0; ch < 16; ch++){

        Row: for(int out_row = 0; out_row < 14; out_row++){

            Col: for(int out_col = 0; out_col < 14; out_col++){

                int in_row = out_row * 2;
                int in_col = out_col * 2;
                data_t maxvalue = input[in_row][in_col][ch];

                POOL_ROW: for(int pool_row = 0; pool_row < 2; pool_row++){

                    POOL_COL: for(int pool_col = 0; pool_col < 2; pool_col++){
